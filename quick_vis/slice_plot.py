...
def create_slice_plot(
    ...
    if args["grid_info"]:
        ...
        data = ds.covering_grid(
            ...
            dims=ds_attributes["dimensions"],
            ds=ds,
        )
        num_cells_0 = float(data[eb_var_name].sum()) if eb_var_name in data else 0
    ...
    if args["normal"] == "y":
        max_res = max(ds_attributes["resolution"][2], ds_attributes["resolution"][0])
...
def main():
    ...
    for ds in ts.piter(dynamic=True):
        ...
        fig = create_slice_plot(
            ds=ds,
            args=args,
            configs=configs,
            slc_center=slc_center,
            vis_field=vis_field,
            axes_unit=axes_unit,
            pbox_width=pbox_width if args["pbox"] else None,
            pbox_center=pbox_center if args["pbox"] else None,
            index=index,
            udf_funcs=udf_funcs,
            eb_var_name=eb_var_name,
            # domain_volume=domain_volume if args["grid_info"] else None,
        )
